...
const FROM = process.env.EMAIL_FROM ?? 'Bodhi Grove Rota <rota@bodhigrove.demo>'

async function send(to: string | string[], subject: string, html: string) {
  if (!resend) {
    log.warn({ action: 'email.send', message: 'RESEND_API_KEY not set, skipping email', subject })
    return
  }
  try {
    await resend.emails.send({ from: FROM, to, subject, html })
  } catch (err) {
    log.error({ action: 'email.send', message: 'Resend rejected the email', subject, err })
    // Don't rethrow — email failures must never break the user-facing action.
  }
}
...
export function sendSignupConfirmation(to: string, duty: string, date: string, time: string, location: string) {
  return send(to, `Rota sign-up confirmed: ${duty}`, wrap(`
...
export function sendSwapDecision(to: string, approved: boolean, duty: string, date: string) {
