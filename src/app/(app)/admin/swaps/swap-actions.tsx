...
export default function SwapActions({ swapId }: { swapId: string }) {
  ...
  if (state && 'success' in state) {
    return <span className="text-xs text-ink/45 italic">Done</span>
  }
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 flex-shrink-0 min-w-[160px]">
      ...
      <textarea
        ...
        className="text-xs border border-sand rounded px-2 py-1 resize-none focus:outline-none focus:ring-1 focus:ring-mist w-full"
      />
      {state && 'error' in state && (
        <p role="alert" className="text-red-600 text-xs">{state.error}</p>
      )}
      <div className="flex gap-1.5">
        ...
        <button
          ...
          value="rejected"
          disabled={pending}
          className="flex-1 text-xs font-medium px-2 py-1.5 bg-white border border-sand text-ink/70 rounded-md hover:bg-paper-100 disabled:opacity-50 transition-colors"
        >
          {pending ? '…' : 'Reject'}
        </button>
      </div>
