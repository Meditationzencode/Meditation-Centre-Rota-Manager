...
export default function ProfileForm({ name, phone }: { name: string; phone: string }) {
  ...
  return (
    <form action={formAction} className="bg-white border border-stone-200 rounded-xl shadow-sm p-6 space-y-4">
      <h2 className="font-serif text-lg border-b border-stone-100 pb-3">Edit Details</h2>

      {error   && <div className="bg-red-50 border border-red-200 text-red-700 text-sm rounded-md px-4 py-3">{error}</div>}
      {success && <div className="bg-sage-50 border border-sage-200 text-sage-700 text-sm rounded-md px-4 py-3">Profile updated successfully.</div>}
      ...
      <div>
        <label className="block text-sm font-medium text-stone-700 mb-1">
          New Password <span className="text-stone-400 font-normal">(leave blank to keep current)</span>
        </label>
